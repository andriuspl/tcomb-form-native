...
class Struct extends Component {
  ...
  isValueNully() {
    return Object.keys(this.myRefs.current).every(ref => this.myRefs.current[ref].isValueNully());
  }

  removeErrors() {
    this.setState({ hasError: false });
    Object.keys(this.myRefs.current).forEach(ref => this.myRefs.current[ref].removeErrors());
  }

  getValue() {
    const value = {};
    for (const ref in this.myRefs.current) {
      value[ref] = this.myRefs.current[ref].getValue();
    }
  ...
  validate() {
    ...
    console.log(Object.keys(this.myRefs.current));
    for (const ref in Object.keys(this.myRefs.current)) {
      console.log(ref);
      if (this.myRefs.current.hasOwnProperty(ref)) {
        console.log(this.myRefs.current[ref]);
        result = this.myRefs.current[ref].validate();
        console.log(result);
      ...
      if (props.hasOwnProperty(prop)) {
        ...
        inputs[prop] = React.createElement(
          getFormComponent(propType, propOptions),
          {
            key: prop,
            ref: (element) => {
              if (!this.myRefs.current) {
                this.myRefs.current = {};
              }
              this.myRefs.current[`${prop}`] = element;
            },
            type: propType,
            options: propOptions,
            value: value[prop],
